...
CREATE	 PROCEDURE [etl].[Cust_FactTicketSalesProcessing_History]
(
...
FROM    dbo.FactTicketSales_History_V2  fts
		...
		JOIN dbo.DimSeason_V2 s ON s.DimSeasonId = fts.DimSeasonId
		JOIN dbo.DimPlan_V2 PL ON pl.dimplanid = fts.dimplanID
WHERE  fts.dimseasonid in (48,43,36)
...
(fts.dimseasonid in (48,43,36) AND pricecode.pricecode LIKE '%INT')
...
--OR
 ...
 (fts.dimseasonid in (48,43,36)AND pricecode.pricecode LIKE '%_JR' AND pl.plancode = 'none')
