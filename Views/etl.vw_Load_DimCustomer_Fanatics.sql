...
, HASHBYTES('sha2_256', ISNULL(RTRIM(ExtAttribute31),'DBNULL_TEXT') 
	...
	--base set
		SELECT
			...
			/*Standard Attributes*/
			, CAST(cust.FAN_ID AS NVARCHAR(50)) [SSID] -- Required
			...
			/*Name*/
			, NULL AS [Prefix]
			, cust.NAME_FIRST AS [FirstName]
			, NULL AS [MiddleName]
			, cust.NAME_LAST AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
			, CASE WHEN ISNULL(cust.ADDRESS2,'')<>'' THEN cust.ADDRESS1+' '+cust.ADDRESS2 WHEN ISNULL(cust.ADDRESS2,'')='' THEN cust.ADDRESS1 END AS [AddressPrimaryStreet]
			, cust.CITY1 AS [AddressPrimaryCity] 
			, cust.REGION_FT AS [AddressPrimaryState] 
			, cust.POSTCODE1 AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(cust.TELNR_LONG,25) AS [PhonePrimary]
...
--		select top 100 *			
		FROM ods.Fanatics_Customer cust WITH (NOLOCK) 
		WHERE 1=1
		AND cust.NAME_FIRST <> 'NHL FAN'
