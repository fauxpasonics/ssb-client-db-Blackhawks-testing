...
CREATE VIEW [etl].[vw_Load_DimCustomer_FormStack] AS 
...
--base set
        SELECT
            'Blackhawks' AS [SourceDB] --Client Source Database such as Raiders or Raiders_Reporting
            , 'FormStack' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            , NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
 
            /*Standard Attributes*/
            , CAST(z.SubmissionID AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
...
 
            /*AddressPrimary*/
            , z.AddressLine1 AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
            , z.AddressLine2 AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!!!!!!!!!!!!!!!
            , z.AddressCity AS [AddressPrimaryCity] 
            , z.AddressState  AS [AddressPrimaryState] 
            , z.AddressZip AS [AddressPrimaryZip] 
...
 
            /*Phone*/
            , LEFT(z.PhoneCell,25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
            , LEFT(z.PhoneHome,25) AS [PhoneHome]
            , LEFT(z.PhoneCell,25) AS [PhoneCell]
            , LEFT(z.PhoneBusiness,25) AS [PhoneBusiness]
...
            , z.SubmissionDateTime AS [SSCreatedDate] --createddate
            , z.SubmissionDateTime AS [SSUpdatedDate] --lastmodifiedbyid
...
        --SELECT COUNT(*)           
        FROM stg.FormStack_Submissions z WITH (NOLOCK) --Change To Source Table
  	    WHERE z.recordcreatedate >= GETDATE()-3
AND z.SubmissionID NOT IN ('399213511','399489357')
    ) a
 
