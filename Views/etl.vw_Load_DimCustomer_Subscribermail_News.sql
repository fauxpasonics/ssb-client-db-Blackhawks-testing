...
CREATE VIEW  [etl].[vw_Load_DimCustomer_Subscribermail_News] AS 
...
            , 'Subscribermail' AS [SourceSystem] --Put source name such as TM or CRM_Contact
            , NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules
 
            /*Standard Attributes*/
            , CAST(z.recipientdata_id AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
...
            /*Name*/
            , NULL AS [Prefix]
            , z.[first Name] AS [FirstName]
            , NULL AS [MiddleName]
            , z.[Last Name] AS [LastName]
            , NULL AS [Suffix]
            , null AS [Fullname]
            --, c.name_title as [Title]
 
            /*AddressPrimary*/
            , z.address AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
...
 
            /*Phone*/
            , LEFT(z.phone ,25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
...
            , TRY_CAST(z.accountid AS INT) [AccountId]
...
            , CASE WHEN ISNULL(TRY_CAST(z.accountid AS BIGINT),'') <>'' THEN CONCAT('TM-', z.accountid) ELSE NULL END AS [Customer_Matchkey] 
...
  		--select count(*)     
        FROM ods.subscribermail_customer_Data_news z-- WITH (NOLOCK) --Change To Source Table
			LEFT JOIN ods.TM_Cust t
			ON TRY_CAST(z.accountid AS BIGINT) = t.acct_id AND t.Primary_code = 'Primary'
		WHERE (z.etl_CreatedOn >= GETDATE()-3 OR z.etl_UpdatedOn >=GETDATE()-3)
